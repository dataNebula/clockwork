...
<div class="app-container">
    <div class="filter-container">
        <div class="box_l">
            <div class="inner_contain">
                <el-row>
                    <el-col :span="8">
                        <el-input class="handle-input" onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))" placeholder="节点ID" size="medium" type="number" v-model="id" clearable />
                    </el-col>
                    <el-col :span="8">
                        <el-input class="handle-input" onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))" placeholder="节点组ID" size="medium" type="number" v-model="nodeGroupId" clearable />
                    </el-col>
                    <el-col :span="8">
                        <el-input class="handle-input" placeholder="节点组名" size="medium" v-model="groupName" clearable />
                    </el-col>
                </el-row>
                <div v-if="showMore">
                    <el-row>
                        ...
                        <el-col :span="8">
                            <el-input class="handle-input" placeholder="域名" size="medium" v-model="domainName" clearable>
                            </el-input>
                        </el-col>
                        <el-col :span="8">
        ...
        <div class="box_r">
            <el-button @click="handleCreate" icon="el-icon-circle-plus-outline" type="primary" size="medium" class="fr">添加节点</el-button>
    ...
    <el-table :data="tableData" append border ref="singleTable" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="组Id" prop="nodeGroupId" width="80"></el-table-column>
        <el-table-column label="组名称" prop="groupName" width="160"></el-table-column>
        ...
        <el-table-column fixed="left" label="操作" width="100">
            <template slot-scope="scope">
                <div class="operate-block">
                    ...
                    <el-popconfirm @onConfirm="handleDelete(scope.$index, scope.row)" icon="el-icon-info" iconColor="red" title="你确定要删除该节点吗？">
    ...
    <div class="pagination">
        <el-pagination :current-page="currentPage" :page-size="pageSize" :page-sizes="[10, 20, 40, 100]" :total="total" @current-change="handleCurrentChange" @size-change="handleSizeChange" layout="total, sizes, prev, pager, next, jumper">
        </el-pagination>
    </div>
    <el-dialog :visible.sync="dialogFormVisible" title="添加节点">
        <el-form :model="node" :rules="rules" label-width="100px" ref="node">
            <el-form-item label="节点组" prop="groupName">
                <el-select v-model="node.nodeGroupId" placeholder="请选择节点组">
        ...
        <div class="dialog-footer" slot="footer">
            ...
            <el-button @click="submitCreate('node')" icon="el-icon-document-add" type="primary">提 交</el-button>
        </div>
    </el-dialog>
    ...
    <el-dialog :visible.sync="dialogEditVisible" title="修改节点">
        <el-form ref="editForm" :model="editForm" :rules="addRules" label-width="100px">
            <el-form-item label="节点组" prop="groupName">
                <el-select v-model="editForm.nodeGroupId" placeholder="请选择节点组">
                    <el-option v-for="item in nodeList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="域名" prop="domainName">
                <el-input v-model="editForm.domainName" placeholder="请输入机器域名"></el-input>
            </el-form-item>
            <el-form-item label="ip" prop="ip">
                <el-input v-model="editForm.ip" placeholder="请输入机器ip"></el-input>
            </el-form-item>
        </el-form>
        <div class="dialog-footer" slot="footer">
            <el-button @click="dialogEditVisible = false">取 消</el-button>
            <el-button @click="submitForm('editForm')" icon="el-icon-document-add" type="primary">提 交</el-button>
        </div>
    </el-dialog>
</div>
</template>
    ...
    methods: {
        getData() {
            ...
            pageParam.condition = JSON.stringify({
                ...
                ip: _self.ip == '' ? null : this.ip,
                domainName: _self.domainName == '' ? null : this.domainName,
                nodeGroupId: _self.nodeGroupId == '' ? null : this.nodeGroupId,
                groupName: _self.groupName == '' ? null : this.groupName,
                status: _self.status == '' ? null : this.status,
                role: "worker"
            });
            _self.tableData = [];
            _self.axios
                ...
                .then(response => {
                    if (response.code === "OK") {
                        ...
                        if (response.data.list) {
                            for (
                                let i = 0; i < response.data.list.length; i++
                            ) {
                                let obj = {};
        ...
        handleEdit(index, row) {
            ...
            this.getAllNodeGroup();
            this.editForm.nodeGroupId=row.nodeGroupId;
        ...
        submitForm(formName) {
            ...
            node.nodeGroupId=this.editForm.nodeGroupId
