...
public final class RetryUtil {
    ...
    private static class Retry {
        public <T> T doRetry(Callable<T> callable, int retryTimes,
                             long sleepTimeInMilliSecond, boolean exponential, List<Class<?>> retryExceptionClasss)
                throws Exception {
            if (null == callable) {
            ...
            for (int i = 0; i < retryTimes; i++) {
                try {
                    ...
                    saveException = e;
                    if (i == 0) {
                        LOG.error(String.format("Exception when calling callable, 异常Msg:%s",
                                saveException.getLocalizedMessage()), saveException);
                    }
                    ...
                    if (i + 1 < retryTimes && sleepTimeInMilliSecond > 0) {
                        ...
                        LOG.error(String.format("Exception when calling callable, "
                        		+ "即将尝试执行第%s次重试.本次重试计划等待[%s]ms,实际等待[%s]ms, 异常Msg:[%s]",
