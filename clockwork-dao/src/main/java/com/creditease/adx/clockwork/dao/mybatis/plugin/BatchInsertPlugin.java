...
/**
 * @ Author     ：XuanDongTang
 * @ Date       ：Created in 11:31 上午 2020/10/28
 * @ Description：
 * @ Modified By：
 */
public class BatchInsertPlugin extends AbstractXmbgPlugin {
    ...
    public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        String objectName = introspectedTable.getRules().calculateAllFieldsClass().toString();
        Method method = new Method(BATCH_INSERT);
        FullyQualifiedJavaType type = new FullyQualifiedJavaType("java.util.List<" + objectName + ">");
        method.addParameter(new Parameter(type, "records"));
    ...
    public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
        ...
        generateActualColumnNamesWithParenthesis(introspectedTable.getAllColumns(), answer);
